...
def writeSubmitScript(cluster="saturn", script_name="abinit.sbatch", job_name="job name", queue=None, nodes=2, cpu=None, hrs=0, mins=30, rundir="simulations"):
    with open(script_name,"w") as fout:
        ...
        fout.write('#SBATCH -N {}\n'.format(nodes))
        if cpu is not None: fout.write('#SBATCH --tasks-per-node={}\n'.format(cpu))
        else: fout.write('#SBATCH --tasks-per-node={}\n'.format(cpus[cluster]))
        ...
        if cpu is not None: fout.write('srun --mpi=pmi2 -n {} abinit < ./input/{}.files > ./results/log\n\n'.format(nodes*cpu,job_name))
        else: fout.write('srun --mpi=pmi2 -n {} abinit < ./input/{}.files > ./results/log\n\n'.format(nodes*cpus[cluster],job_name))
